...
def events_to_voxel_all(events, seq_len, num_bins, width, height, device): #frame_nums
...
class DVS_Lip(Dataset):
    ...
    def __getitem__(self, index):
        ...
        events_input = events_input[np.where((events_input[:,0] >= 112) & (events_input[:,0] < 208) & (events_input[:,1] >= 72) & (events_input[:,1] < 168))] #16, 112, 16, 112
        ...
        event_voxel_low = events_to_voxel_all(events_input, self.length, self.args.num_bins[0], 96, 96, device='cpu') # (30*num_bins[0], 96, 96)
        event_voxel_high = events_to_voxel_all(events_input, self.length, self.args.num_bins[1], 96, 96, device='cpu') # (30*num_bins[1], 96, 96)

        # data augmentation
        if self.phase == 'train':
            event_voxel_low, event_voxel_high = RandomCrop(event_voxel_low, event_voxel_high, (88, 88)) #event_voxel_high, (88, 88)
            event_voxel_low, event_voxel_high = HorizontalFlip(event_voxel_low, event_voxel_high)
        else:
            event_voxel_low, event_voxel_high = CenterCrop(event_voxel_low, event_voxel_high, (88, 88)) #event_voxel_high, (88, 88)
